...
import locale
API_KEY = "5ab28d91e1ad47a21ab998b83c12b34f"
...
class Weather():
    ...
    def __init__(self, *args, **kwargs):
        if kwargs.get("country") and kwargs.get("city"):
            self.url = "{0}?{1}&appid={2}&units=metric&lang=ru".format(URL,
            f"q={kwargs['city']},{kwargs['country']}", API_KEY)
        elif kwargs.get("latitude") and kwargs.get("longitude"):
            self.url = "{0}?{1}&appid={2}&units=metric&lang=ru".format(URL,
            f"lat={kwargs['latitude']}&lon={kwargs['longitude']}", API_KEY)
        else:
            self.url = "{0}?{1}&appid{2}&units=metric&lang=ru".format(
            URL,"q=London,UK", API_KEY)
    ...
    def create_response(self):
        ...
        Погода в {self.result["name"]}
        {self.result["sys"]["country"]}:
        {self.result["weather"][0]["main"]},
        {self.result["weather"][0]["description"]}.
...
if __name__ == '__main__':
    ...
    print(obj.result)
    obj.create_response()
    #obj2 = Weather(latitude=3.28, longitude=-0.28)
    #print(obj2.url)
    #obj3 = Weather()
    #print(obj3.url)
    #obj3.get_date()
